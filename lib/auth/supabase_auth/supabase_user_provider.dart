...
class GetItSupabaseUser extends BaseAuthUser {
  GetItSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> getItSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = GetItSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
